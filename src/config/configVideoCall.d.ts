declare interface ZegoCloudRoomConfig {
  ...
  onInRoomCommandReceived?: (fromUser: ZegoUser, command: string) => void; // Callback for in-room custom messages received.
  onInRoomTextMessageReceived?: (
    messages: ZegoSignalingInRoomTextMessage[],
  ) => void; // Callback for in-room signaling text messages received.
}
